...
const Replaces = () => {
  ...
  const sortJSON = (dataJS) => {
    let clusters = {};
    const data = dataJS.data.result;

    //создаем массив уникальный значений по параметру wsrep_cluster_name
    let result = data.reduce(function (acc, item) {
      if (acc.indexOf(item.metric.wsrep_cluster_name) === -1) {
          return acc.concat(item.metric.wsrep_cluster_name);
      }
      return acc;
  }, []);
    ...
    console.log("clusters1", clusters);

    // проходим по исходным данным и если ключи объектов совпали
    // по ключу wsrep_cluster_name, добавляем в массив объектов
    // свойство node_name
    data.forEach(function (item) {
      if (clusters.hasOwnProperty(item.metric.wsrep_cluster_name)) {
        clusters[item.metric.wsrep_cluster_name].push(item.metric.node_name);
      }
    });

    console.log("clusters2", clusters);

    // Преобразуем объект clusterNames обратно в массив с объединением имен
    let dataResult = result.map(function (cluster) {
      return {
        'clustername': cluster,
        'nodename': clusters[cluster].length > 0 ? clusters[cluster].join('|') : 'undefined'
    }});
